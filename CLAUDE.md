...
> **Current state:** MVP + V1 are essentially complete and **V2 is progressing**; the UI is on the Claude Design system. **Chapter AND book-level automation** have shipped (`POST /ai/chapters/{id}/generate` + `POST /ai/books/{id}/generate` → RQ jobs producing one human-approved Revision per scene, with per-chapter review and **job cancellation**). A 2026-07 **gap-fix program** also landed: brainstorm/expand/compress AI ops, **Codex relations + progression tab UIs**, a scene beat editor, real command-palette search, editor undo/redo, **cover-embedded EPUB/PDF exports**, an Ollama health-ping, an idempotent Hungarian demo seed (`python -m app.seed`), and an auto-migrating docker entrypoint. Earlier highlights: DESIGN-C net-new screens + a celestial "delight" pass; PDF export (WeasyPrint); a real Prompt Library (CRUD + seeded builtins); `scene_count` + `/auth/me`; in-app Ollama model pull; CodexProgression filtering the AI context ("state as of scene N"). One screen remains a placeholder (Hangok). Test suites: **2102 tests** (web 1001 · api 582 · ai 519). Several "must not include yet" items below have since shipped — the **authoritative live status is [docs/17](docs/17_status_and_roadmap.md)** (it overrides anything stale here). See also the design system [docs/09](docs/09_design_system_novelcrafter_inspired.md), the UI/screen inventory [docs/07](docs/07_ui_ux_routes_and_components.md), and the design rollout [docs/18](docs/18_design_rollout_and_remaining.md).
...
> **Since shipped (V1/V2 — this list is the original MVP scope):** **image generation** (Codex/character + book-cover generation, Phase 1–2), **PDF export** (WeasyPrint), and **full-book automatic generation** at both **chapter and book level** (scene-by-scene from beats as RQ jobs, one human-approved Revision per scene, per-chapter review + job cancellation). The rest (collaboration, payments, teams, mobile, advanced KDP, public SaaS, complex permissions, multiplayer) remain out of scope. Live status: [docs/17](docs/17_status_and_roadmap.md).
