"""Cross-resource integration tests for the AI service workflow.

Covers the AI-owned side of the generation contract: AI endpoints produce
unapproved Revisions + a GenerationJob, the describe flow returns one revision
per sensory channel, and generation jobs are readable/filterable.

The human-in-the-loop approve/reject (which mutates Scene) and export live in
``apps/api`` and are covered by that service's revision + export suites — this
file deliberately does not reach across the service boundary.

All AI calls are mocked via FastAPI dependency override — no real LLM needed.
``scene_id`` is a synthetic UUID: with the AI service mocked, the endpoint never
validates the scene against the DB, so no domain setup call is required here.
"""
import uuid
...
async def test_rewrite_creates_unapproved_revision(
    client: AsyncClient, auth_headers: dict, mock_ai
):
...
async def test_describe_returns_6_channel_revisions(
    client: AsyncClient, auth_headers: dict, mock_ai
):
    scene_id = str(uuid.uuid4())
    resp = await client.post(
...
async def test_generation_job_stored_in_db(
    client: AsyncClient, auth_headers: dict, db_session, mock_ai
):
...
async def test_jobs_list_filterable_by_status(
    client: AsyncClient, auth_headers: dict, db_session, mock_ai
):
