 ...
 * radius, popover shadow, 13px text, a leading type icon and the `woaToastPop`
 * spring entrance. Positioned bottom-right.
 *
...
export function Toaster() {
  return (
    <SonnerToaster
      ...
      toastOptions={{
        // The shell: surface card, default (info-ish) accent edge, spring pop.
        // Type variants below override the left border colour.
        classNames: {
          toast:
            "!flex !items-center !gap-2 !rounded-[10px] !border !border-border " +
            "!border-l-[3px] !bg-surface !px-3.5 !py-2.5 !text-[13px] !text-text " +
            "!shadow-popover [animation:woaToastPop_.4s_cubic-bezier(.34,1.56,.64,1)] " +
            "!font-sans",
